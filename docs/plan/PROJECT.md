...
| 功能 | 说明 |
|------|------|
| 健康检查 | 定期校验文件完整性，`hit check` / `hit repair` |
| 镜像源管理 | 内置中国镜像，自动速度测试与切换 |
| 软件束（Bundle） | 一键安装多个软件，适合团队标准化环境 |
| 沙盒环境（Shadow） | 隔离运行时环境 |
| 生命周期管理 | 归档、孤立文件清理、跨软件去重 |
| 运行时监控 | `hit top` / `hit ps` / `hit trace` |
| 插件系统 | Lua 脚本引擎、插件钩子 |
| 配置同步 | 跨设备同步配置和已安装列表 |
| 开发模式 | 本地目录安装，文件变化自动重载 |
| 备份与恢复 | 完整配置备份 |
| 增量更新 | 仅下载差异部分 |
| 环境诊断 | `hit doctor` 一键排查 |
